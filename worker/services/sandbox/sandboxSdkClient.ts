...
export class SandboxSdkClient extends BaseSandboxService {
	...
	private async writeBinaryFileViaBase64(
		targetPath: string,
		data: ArrayBuffer,
		bytesPerChunk: number = 16 * 1024,
	): Promise<void> {
	...
	async downloadTemplate(
		...
		// 	? `${downloadDir}/${templateName}-v2.zip`
		// 	: `${templateName}-v2.zip`;
	...
	private async ensureTemplateExists(
		...
		if (!(await this.checkTemplateExists(templateName))) {
			...
			this.logger.info(
				`Wrote zip file to sandbox in chunks: ${templateName}.zip`,
			);
			...
			try {
				...
				if (metadata.processId) {
					try {
						...
						this.logger.error(
							`Process ${metadata.processId} not found or not running`,
						);
