...
export const apps = sqliteTable('apps', {
    ...
    customDomain: text('custom_domain'), // User's custom domain (e.g., app.customer.com)
    customDomainStatus: text('custom_domain_status', {
        enum: ['pending', 'active', 'failed', 'moved']
    }).default('pending'), // Status of custom domain verification
    customDomainVerificationErrors: text('custom_domain_verification_errors', {
        mode: 'json'
    }), // Array of verification errors
    customHostnameId: text('custom_hostname_id'), // Cloudflare custom hostname ID
    customDomainCreatedAt: integer('custom_domain_created_at', {
        mode: 'timestamp'
    }), // When custom domain was created
    customDomainUpdatedAt: integer('custom_domain_updated_at', {
        mode: 'timestamp'
    }), // When custom domain was last updated
	...
	// Custom Domains
	customDomainIdx: index('apps_custom_domain_idx').on(table.customDomain),
    customHostnameIdIdx: index('apps_custom_hostname_id_idx').on(table.customHostnameId),
    customDomainStatusIdx: index('apps_custom_domain_status_idx').on(table.customDomainStatus),
